...
export async function createQuestion (data) {
  const { version, label, type, options } = data;
...
export async function getQuestionsLatest (ids) {
  const questions = await db.raw(`
    ...
    SELECT
      ...
      questions.options
    from latest_version
    INNER JOIN questions ON questions.id = latest_version.id
      AND questions.version = latest_version.version;
  `,
  [ids]
  );

  // Preserve original question order from ids
  return ids.map(id => {
    return find(questions.rows, q => q.id === id);
  });
}

export async function updateQuestion (data) {
  const { id, version, label, type, options } = data;
