...
exports.v1 = function (settings) {
  ...
  return async function (request, h) {
    ...
    if (request.query.error === 'access_denied' || request.query.denied) {
      return h.unauthenticated(Boom.internal('Application rejected'), {
        credentials
      });
    }
    ...
    if (protocol !== 'https' && settings.isSecure) {
      return h.unauthenticated(
        Boom.internal(
          'Invalid setting  - isSecure must be set to false for non-https server'
        ),
        { credentials }
      );
    }
    ...
    if (!request.query.oauth_token) {
      ...
      const authQuery = internals.resolveProviderParams(
        request,
      ...
      return h
        .redirect(
          settings.provider.auth + '?' + internals.queryString(authQuery)
        )
        .takeover();
    }
    ...
    if (!request.query.oauth_verifier) {
      return h.unauthenticated(
        Boom.internal(
          'Missing verifier parameter in ' + name + ' authorization response'
        ),
        { credentials }
      );
    }
    ...
    if (!state) {
      return internals.refreshRedirect(
        request,
        name,
        protocol,
        settings,
    ...
    if (request.query.oauth_token !== state.token) {
      return h.unauthenticated(
        Boom.internal(name + ' authorized request token mismatch'),
        { credentials }
      );
    }
    ...
    try {
      var { payload: token } = await client.token(
        state.token,
    ...
    const get = async (uri, params = {}) => {
      ...
      const { payload: resource } = await client.resource('get', uri, params, {
...
exports.v2 = function (settings) {
  return async function (request, h) {
    ...
    if (request.query.error === 'access_denied' || request.query.denied) {
      return h.unauthenticated(
        Boom.internal(
          `App rejected: ${request.query.error_description ||
    ...
    if (!request.query.code) {
      ...
      const query = internals.resolveProviderParams(
        request,
        settings.providerParams
      );
      ...
      query.redirect_uri = internals.location(
        request,
        protocol,
        settings.location
      );
      ...
      if (settings.provider.pkce) {
        ...
        if (settings.provider.pkce === 'S256') {
          ...
          query.code_challenge = internals.createCodeChallenge(
            state.codeVerifier
          );
      ...
      h.state(cookie, state);
      return h
        .redirect(settings.provider.auth + '?' + internals.queryString(query))
        .takeover();
    }
    ...
    if (
      ...
      requestState.substr(
        0,
        Math.min(requestState.length, internals.nonceLength)
      )
    ) {
      return h.unauthenticated(
        Boom.internal('Incorrect ' + name + ' state parameter'),
        { credentials }
      );
    }
    ...
    try {
      var { res: tokenRes, payload } = await Wreck.post(
        settings.provider.token,
        requestOptions
      );
    } catch (err) {
      return h.unauthenticated(
        Boom.internal('Failed obtaining ' + name + ' access token', err),
        { credentials }
      );
    }
    ...
    if (tokenRes.statusCode < 200 || tokenRes.statusCode > 299) {
      return h.unauthenticated(
        Boom.internal('Failed obtaining ' + name + ' access token', payload),
        { credentials }
      );
    }
    ...
    try {
      ...
      return h.unauthenticated(
        Boom.internal(
          'Received invalid payload from ' + name + ' access token endpoint',
          payload
        ),
        { credentials }
      );
    ...
    const get = async (uri, params = {}) => {
      ...
      const getQuery = Object.keys(params).length
      ...
      if (res.statusCode !== 200) {
        throw Boom.internal(
          'Failed obtaining ' + name + ' user profile',
          response
        );
      }
      ...
      try {
        ...
        Bounce.rethrow(err, 'system');
        throw Boom.internal(
          'Received invalid payload from ' + name + ' user profile',
          response
        );
      }
...
internals.refreshRedirect = function (
  request,
  name,
  protocol,
  settings,
  ...
  if (request.query.refresh) {
    return h.unauthenticated(
      Boom.internal('Missing ' + name + ' request token cookie'),
      { credentials }
    );
  }
  ...
  const refreshUrl =
    internals.location(request, protocol, settings.location) +
    '?' +
    internals.queryString(refreshQuery);
  return h
    .response(
      `<html><head><meta http-equiv="refresh" content="0;URL='${refreshUrl}'"></head><body></body></html>`
    )
    .takeover();
};
...
exports.Client = internals.Client = function (options) {
  ...
  this.settings = {
    ...
    clientSecret:
      options.provider.signatureMethod === 'RSA-SHA1'
        ? options.clientSecret
        : internals.encode(options.clientSecret || '') + '&',
...
internals.Client.prototype.token = function (
  oauthToken,
  oauthVerifier,
  tokenSecret
) {
  ...
  return this._request('post', this.settings.token, null, oauth, {
    secret: tokenSecret,
    desc: 'token credentials'
  });
};
...
internals.Client.prototype._request = async function (
  method,
  uri,
  params,
  oauth,
  ...
  if (result.statusCode !== 200) {
    throw Object.assign(
      Boom.internal(
        `Failed obtaining ${this.provider} ${desc}`,
        result.payload
      ),
      result
    );
  ...
  if (!options.raw) {
    try {
      ...
      throw Object.assign(
        Boom.internal(
          `Received invalid payload from ${this.provider} ${desc} endpoint`,
          result.payload
        ),
        result
      );
...
internals.Client.header = function (oauth) {
  ...
  for (let i = 0; i < names.length; ++i) {
    const name = names[i];
    header +=
      (i ? ', ' : '') + name + '="' + internals.encode(oauth[name]) + '"';
  }
...
internals.Client.baseUri = function (uri) {
  ...
  const isDefaultPort =
    resource.port &&
    ((protocol === 'http:' && resource.port === '80') ||
      (protocol === 'https:' && resource.port === '443'));
  return (
    protocol +
    '//' +
    resource.hostname.toLowerCase() +
    (isDefaultPort || !resource.port ? '' : ':' + resource.port) +
    resource.pathname
  );
};

internals.Client.prototype.signature = function (
  method,
  baseUri,
  params,
  oauth,
  ...
  for (let i = 0; i < normalized.length; ++i) {
    normalizedParam +=
      (i ? '&' : '') + normalized[i][0] + '=' + normalized[i][1];
  }

  // String Construction (3.4.1.1)

  const baseString =
    internals.encode(method.toUpperCase()) +
    '&' +
    internals.encode(baseUri) +
    '&' +
    internals.encode(normalizedParam);

  if (oauth.oauth_signature_method === 'RSA-SHA1') {
    // RSA-SHA1 (3.4.3)
    return Crypto.createSign('sha1')
      .update(baseString)
      .sign(this.settings.clientSecret, 'base64');
  }

  // HMAC-SHA1 (3.4.2)

  const key = tokenSecret
    ? this.settings.clientSecret + internals.encode(tokenSecret)
...
internals.createCodeVerifier = function () {
  return Cryptiles.randomString(internals.codeVerifierLength)
    .replace(/_/g, '.')
    .replace(/=/g, '~');
};
...
internals.getProtocol = function (request, settings) {
  ...
  const location = internals.location(
    request,
